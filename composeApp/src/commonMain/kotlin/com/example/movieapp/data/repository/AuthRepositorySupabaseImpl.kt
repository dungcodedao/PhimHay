 ...
 */
class AuthRepositorySupabaseImpl(
    ...
    override suspend fun updateProfile(username: String?, avatarUrl: String?): Resource<Unit> {
        return try {
            ...
            Resource.Error(e.message ?: "Cập nhật profile thất bại")
        }
    }
    ...
    override suspend fun uploadAvatar(byteArray: ByteArray, fileName: String): Resource<String> {
        return try {
            ...
            Resource.Error(e.message ?: "Tải ảnh lên thất bại")
        }
    }

    override fun getUsername(): String? =
        supabaseAuth.currentUserOrNull()?.userMetadata?.get("username")
            ?.toString()?.removeSurrounding("\"")

    override fun getAvatarUrl(): String? =
        supabaseAuth.currentUserOrNull()?.userMetadata?.get("avatar_url")
            ?.toString()?.removeSurrounding("\"")
}
